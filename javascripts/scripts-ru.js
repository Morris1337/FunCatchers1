...
function sendRequest() {
	...
	jQuery('#contactWrapper').append('<div class="success"><h4>Письмо успешно отправлено!</h4><br><p>Спасибо что использовали нашу контактную форму. <strong>'+name+'</strong>!</p></div>');
	}
}
function sendRequest_news() {
	...
	jQuery('.intro-form-wrap').append('<div class="success"><h4>Подписка успешно отправлена!</h4><br><p>Спасибо за использование нашей рассылки, <strong>'+name+'</strong>! Ваш адрес электронной почты зарегистрирован.</p></div>');
	}
}
	...
	var errors=0;
	 ...
	 if($j('#contactform #name').val()=='') {
		...
		var hasClass=$j('#contactform #email').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #email').parent().append('<label for="contactname" generated="true" class="error">Пожалуйста, введите действительный адрес электронной почты</label>');	
		...
		var hasClass=$j('#contactform #phone').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #phone').parent().append('<label for="contactname" generated="true" class="error">Пожалуйста, введите действующий телефонный номер</label>');	
		...
		var hasClass=$j('#contactform #subject').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #subject').parent().append('<label for="contactname" generated="true" class="error">Вам нужно ввести тему!</label>');	
		...
		var hasClass=$j('#contactform #body').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #body').parent().append('<label for="contactname" generated="true" class="error">Вам необходимо ввести сообщение!</label>');	
...
function check_values_news() {
	...
	var errors=0;
	 ...
	 if($j('#newsletterform #name_news').val()=='') {
	 	...
	 	if(!hasClass)
	 	    $j('#newsletterform #name_news').parent().append('<label for="contactname" generated="true" class="error">Пожалуйста, введите Ваше имя</label>');
		...
		var hasClass=$j('#newsletterform #email_news').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#newsletterform #email_news').parent().append('<label for="contactname" generated="true" class="error">Пожалуйста, введите действительный адрес электронной почты</label>');	
