        ...
        <section class="section-wrapper very-top-padding">
          <div class="background-inner1"><!-- add parallax background -->
              ...
              <img src="images/shadow-top.png" class="shadow-top" alt="shadow">  
                ...
                <div class="container">
                <h2 class="section-title"><?php echo $lang['publications']?><br /><span><?php echo $lang['pubfuturepast']?></span></h2>
                
                    
       <table>
        <tr>
          <th style="background-color: #FF8C00">
            <?php echo $lang['futureevents']?>
          </th>
          <th style="background-color: #32CD32">
            <?php echo $lang['today']?>
          </th>
          <th style="background-color: #F62217">
            <?php echo $lang['pastevents']?>
          </th>
        </tr>
       </table>
...
$sql = "SELECT * FROM `events` WHERE ? BETWEEN eventdate AND eventdate2";
$parameters = array($todaydate);
$statement = $db->prepare($sql);
$statement->execute($parameters);

     
while ($row = $statement->fetch()){ ?>
<h6 class="blink" style="text-align:center; color: black">
    <?php echo $row['eventdate']; ?>
</h6>     
  <table style='border-collapse: collapse; width: 50%; margin-right: 25%; margin-right: 25%;'>
  ...
  $imageURL = "images/post/".$row['filename']?>
      ...
      <tr>
          <td>
            <a href="<?php echo 'post'.$row['id'].'.php'?>"><?php echo $lang['readmore']?></a> 
          </td>
      </tr>
      <tr style="background-color: #32CD32">
          <td>
            <a href="<?php echo 'post'.$row['id'].'.php'?>"><img src="<?php echo $imageURL?>"></a>
          </td>
      </tr>
...
$db = connectDatabase();
$todaydate = date("Y-m-d");
$sql = "SELECT * FROM `events` WHERE ? < eventdate AND ? < eventdate2 ORDER BY eventdate ASC";
$parameters = array($todaydate, $todaydate);
$statement = $db->prepare($sql);
$statement->execute($parameters);

     
while ($row = $statement->fetch()){ ?>
<h6 style="float:left; color: black">
    <?php echo $row['eventdate']; ?>
</h6>
<br>     
  <table style='border-collapse: collapse; width: 50%; margin-right: 50%;'>
  <?php $row = array_filter($row, static function ($element) {
    return $element !== "";
    //                   ↑
    // Array value which you want to delete     
...
$db = connectDatabase();
$todaydate = date("Y-m-d");
$sql = "SELECT * FROM `events` WHERE ? > eventdate AND ? > eventdate2 ORDER BY eventdate DESC";
$parameters = array($todaydate, $todaydate);
$statement = $db->prepare($sql);
$statement->execute($parameters);
...
while ($row = $statement->fetch()){ ?>
<h6 style="float:right; color: black">
    <?php echo $row['eventdate']; ?>
</h6>
<br>     
  <table style='border-collapse: collapse; width: 50%; margin-left: 50%;'>
  <?php $row = array_filter($row, static function ($element) {
    return $element !== "";
    //                   ↑
    // Array value which you want to delete     
  ...
  // Get images from the database
  $imageURL = "images/post/".$row['filename']?>
      <tr style="background-color: #F62217">
          <th>
             <?php echo($row['title']); ?>
          </th>
      </tr>
